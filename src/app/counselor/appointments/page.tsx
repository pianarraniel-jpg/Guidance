      ...
      <header className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-8">
        <div>
          <div className="flex items-center gap-3 mb-2">
            <h1 className="text-3xl font-black text-slate-900 tracking-tight">Appointment Queue</h1>
        ...
        <CardContent className="p-0">
          <Table>
            <TableHeader className="bg-slate-50/50">
              <TableRow className="border-b border-slate-100 hover:bg-transparent">
                ...
                <TableHead className="font-bold text-slate-400 text-[10px] uppercase tracking-widest h-14">Session Type</TableHead>
                  ...
                  <TableCell className="pl-8 py-5">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="text-[10px] text-slate-400 font-bold uppercase">#{app.id.slice(-4).toUpperCase()}</span>
                      </div>
                    </div>
                  </TableCell>
                  <TableCell>
                    ...
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-56 rounded-2xl p-2 shadow-xl border-slate-100">
                          <DropdownMenuItem onSelect={() => openDetails(app)} className="flex items-center gap-2 p-3 rounded-xl cursor-pointer font-bold text-xs text-slate-700 hover:bg-slate-50">
                            <Eye className="h-4 w-4 text-slate-400" /> View Details
                          </DropdownMenuItem>
                          <DropdownMenuSeparator className="my-1 bg-slate-50" />
                          {app.status === APPOINTMENT_STATUS.PENDING && (
                            <DropdownMenuItem onSelect={() => handleUpdateStatus(app.id, APPOINTMENT_STATUS.CONFIRMED)} className="flex items-center gap-2 p-3 rounded-xl cursor-pointer font-bold text-xs hover:bg-emerald-50 hover:text-emerald-700">
                              <Check className="h-4 w-4" /> Accept Session
                            </DropdownMenuItem>
                          )}
                          {app.status === APPOINTMENT_STATUS.CONFIRMED && (
                            <DropdownMenuItem onSelect={() => handleUpdateStatus(app.id, 'completed')} className="flex items-center gap-2 p-3 rounded-xl cursor-pointer font-bold text-xs hover:bg-blue-50 hover:text-blue-700">
                              <CheckCircle2 className="h-4 w-4" /> Complete Session
                            </DropdownMenuItem>
                          )}
                          <DropdownMenuItem onSelect={() => handleUpdateStatus(app.id, 'pending')} className="flex items-center gap-2 p-3 rounded-xl cursor-pointer font-bold text-xs hover:bg-blue-50 hover:text-blue-700">
                            <CalendarClock className="h-4 w-4" /> Reschedule
                          </DropdownMenuItem>
                          <DropdownMenuSeparator className="my-1 bg-slate-50" />
                          <DropdownMenuItem onSelect={() => handleDelete(app.id)} className="flex items-center gap-2 p-3 rounded-xl cursor-pointer font-bold text-xs text-red-500 hover:bg-red-50 hover:text-red-700">
          ...
          <div className="p-6 border-t border-slate-50 flex items-center justify-between">
            <span className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">
              Queue: {filteredAppointments.length} sessions • {pendingCount} awaiting approval
            </span>
          </div>
        </CardContent>
      </Card>
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  <Badge className="bg-primary/10 text-primary border-none font-bold text-[10px] uppercase">#{selectedApp?.id?.slice(-6).toUpperCase()}</Badge>
          ...
          <div className="p-8 space-y-8">
            <div className="grid grid-cols-2 gap-8">
              ...
              <div className="space-y-1.5">
                ...
                <p className="text-sm font-bold text-slate-700">{selectedApp?.location || 'Guidance Office - Room 302'}</p>
              </div>
              <div className="space-y-1.5">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
            ...
            <div className="flex gap-3 pt-4 border-t">
              {selectedApp?.status === 'pending' && (
                <Button onClick={() => { handleUpdateStatus(selectedApp.id, APPOINTMENT_STATUS.CONFIRMED); setIsDetailsOpen(false); }} className="flex-1 h-12 bg-primary hover:bg-primary/90 rounded-xl font-black gap-2">
                  <Check className="h-4 w-4" /> Accept Session
                </Button>
              )}
              {selectedApp?.status === 'confirmed' && (
                <Button onClick={() => { handleUpdateStatus(selectedApp.id, 'completed'); setIsDetailsOpen(false); }} className="flex-1 h-12 bg-emerald-600 hover:bg-emerald-700 rounded-xl font-black gap-2">
                  <CheckCircle2 className="h-4 w-4" /> Mark as Complete
                </Button>
              )}
              <Button variant="outline" onClick={() => setIsDetailsOpen(false)} className="flex-1 h-12 rounded-xl font-bold border-slate-200">
                Close
              </Button>
            </div>
          </div>
