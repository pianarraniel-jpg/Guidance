...
export default function CounselorDashboard() {
  ...
  const [analyticsTab, setAnalyticsTab] = useState<"volume" | "concerns">(
    "volume",
  );

  const loadData = React.useCallback(async () => {
    const [allApts, allAssessments, alertsRes] = await Promise.all([
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'chat_alerts' }, (payload) => {
        ...
        toast({
          variant: 'destructive',
          title: `High-Risk Alert — ${alert.student_name}`,
          description: `Triggered by: "${alert.trigger_phrase}". Go to Chat Monitor to review.`,
          duration: 8000,
        });
      })
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'chat_alerts' }, () => {
        loadData();
      })
      .subscribe();
  ...
  const completedWeeksSessions = thisWeeksSessions.filter(
    (a) => a.status === "completed",
  );
  const weeklyCompletionRate = Math.min(
    100,
    Math.round((completedWeeksSessions.length / 10) * 100),
  );
  ...
  const stats = [
    ...
    },
    {
      ...
      value: assessments.filter((a) => a.stressLevel > 75).length.toString(),
      icon: AlertCircle,
      color: "text-red-600",
      bg: "bg-red-50",
      badge: "Review",
    ...
    <div className="p-8 max-w-7xl mx-auto w-full">
      ...
      <header className="mb-10">
        ...
        <p className="text-muted-foreground font-medium">
          Managing campus wellness and student synchronize schedules for{" "}
          ...
          >
            <CardContent className="p-6">
              <div className="flex justify-between items-start mb-4">
                ...
                <Badge
                  variant="outline"
                  className="text-[10px] font-black uppercase tracking-widest border-slate-100 text-muted-foreground bg-slate-50/50"
                >
                  {stat.badge}
                </Badge>
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <CardDescription className="text-xs mt-0.5">
                    Real-time consultation trends and wellness breakdown.
                  </CardDescription>
                </div>
              </div>
            ...
            <CardContent className="p-8 h-[340px]">
              {analyticsTab === "volume" ? (
                sessionVolumeData.some((d) => d["Total Sessions"] > 0) ? (
                  ...
                  <div className="h-full flex flex-col items-center justify-center gap-3 text-center">
                    ...
                    <p className="text-sm font-bold text-slate-400">
                      No session volume logged this week
                    </p>
                    <p className="text-xs text-slate-300">
                      New appointments scheduled on GuidanceSync will show up
                      here automatically.
                    </p>
                  </div>
                )
                ...
                <div className="h-full flex flex-col items-center justify-center gap-3 text-center">
                  ...
                  <p className="text-sm font-bold text-slate-400">
                    No category concerns breakdown yet
                  </p>
                  <p className="text-xs text-slate-300">
                    Complete student consultation profiles to display your
                    primary wellness segments.
                  </p>
                </div>
              )}
            </CardContent>
                ...
                {todaySessions.length > 0 ? (
                  todaySessions.map((item, idx) => (
                    ...
                    >
                      ...
                      <div
                        className={`relative z-10 h-8 w-8 rounded-full border-4 border-white shadow-sm flex items-center justify-center ${idx === 0 ? "bg-primary" : "bg-slate-200"}`}
                      >
                        {idx === 0 ? (
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-3 mb-1">
                          ...
                          <Badge
                            className={`text-[9px] font-black uppercase tracking-tighter ${item.status === "confirmed" ? "text-emerald-500 bg-emerald-50" : "text-blue-500 bg-blue-50"} border-none`}
                          >
                            {item.status}
                          </Badge>
                        </div>
        ...
        <div className="lg:col-span-4 space-y-6">
          <Card className="border-0 shadow-lg shadow-slate-200/40 bg-white rounded-3xl overflow-hidden">
            <CardHeader className="p-6 pb-2 flex flex-row items-center justify-between">
              ...
              <Badge className="bg-primary text-white border-none font-black text-[9px] px-2 h-5">
                {pendingCount} New
              </Badge>
            </CardHeader>
          ...
          <Card className="border-0 shadow-lg shadow-slate-200/40 bg-white rounded-3xl p-6 relative overflow-hidden group">
            <div className="flex items-center justify-between mb-4">
              ...
              <Badge className="bg-indigo-50 text-indigo-700 border-none font-black text-[9px] px-2 h-5">
                Real-time
              </Badge>
            </div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center text-xs font-bold">
                  ...
                  <span className="text-slate-800">
                    {completedWeeksSessions.length} / 10 Sessions
                  </span>
                </div>
                <div className="w-full bg-slate-100 rounded-full h-2">
                  <div
                    ...
                    style={{
                      width: `${Math.min(100, Math.round((completedWeeksSessions.length / 10) * 100))}%`,
                    }}
                  />
                </div>
                <p className="text-[10px] text-slate-400 font-bold leading-tight pt-1">
                  Completed {completedWeeksSessions.length} counselor
                  consultations against recommended target.
                </p>
              </div>
              <Button
                asChild
                variant="outline"
                className="w-full rounded-xl border-slate-200 text-xs font-black h-10 hover:bg-slate-50"
              >
                <Link
                  href="/counselor/session-records"
                  className="flex items-center justify-center gap-1"
                >
                  View Detailed Records{" "}
                  <ChevronRight className="h-4 w-4 text-slate-400 group-hover:translate-x-0.5 transition-transform animate-none" />
                </Link>
              </Button>
            </div>
          </Card>
          ...
          <Card className="border-0 shadow-lg shadow-slate-200/40 bg-primary rounded-3xl p-6 text-white relative overflow-hidden group">
            <div className="relative z-10">
              ...
              <Button
                asChild
                variant="secondary"
                className="w-full bg-white text-primary font-black rounded-xl"
              >
